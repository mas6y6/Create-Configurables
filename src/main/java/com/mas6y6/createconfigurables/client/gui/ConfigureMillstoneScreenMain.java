...
public class ConfigureMillstoneScreenMain extends Screen {
    ...
    protected void init() {
        ...
        scrollList.setOnSelectEntry((ctx) -> {
            Minecraft.getInstance().setScreen(new ConfigureMillstoneScreenRecipe(controller_uuid.toString(),ctx.entry().items().getFirst(), recipeConflicts, config));
        });
        ...
        this.resetButton = new TextureButton(this.leftPos + 136, this.topPos + 151, 18, 18, ResourceLocation.parse("createconfigurables:textures/gui/buttons.png"), (button) -> {
            PacketDistributor.sendToServer(new ServerBoundSetMillstoneConfigurationPacket(new MillstoneConfig(Map.of(), controller_uuid)));
        ...
        PacketDistributor.sendToServer(new ServerBoundGetMillstoneConflictingRecipesPacket());
        PacketDistributor.sendToServer(new ServerBoundGetMillstoneConfigPacket(controller_uuid));
    ...
    public void render(@NotNull GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.renderItem(AllBlocks.MILLSTONE.asStack(), 0, 0);
        poseStack.popPose();
