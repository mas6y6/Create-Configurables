...
public class ItemDrainConfig {
    public Map<ResourceLocation, ResourceLocation> config;
    public UUID uuid;

    public ItemDrainConfig(Map<ResourceLocation, ResourceLocation> config, UUID uuid) {
        this.config = config;
        this.uuid = uuid;
    }

    public static StreamCodec<FriendlyByteBuf, ItemDrainConfig> STREAM_CODEC = StreamCodec.of(
            (buf, config) -> {
            ...
            (buf) -> {
                ...
                return new ItemDrainConfig(config, uuid);
            }
    );
